...
const PopupCurdPricing = (props) => {
    ...
    const addNewHandler = () => {
        ...
        //az redux migiri post mikoni
        let f = {};
        f.COC_SRL = props.PricingItem.COC_SRL;
        f.PROFORM_SRL = props.PoSrl;
        f.CERTFICATE_NO_CHK = props.CertificateNo;
        f.ADJ_CHK = props.AdjChk;

        baseService.post("/PricingResid/Create_Pricing_List", f).then(({ data }) => {

            dispatch(snackbarActions.success("لیست قیمت گذاری با موفقیت ایجاد گردید."))
        })
                    ...
                    toolbar={
                        (
                            <PortletHeaderToolbar>
                                {props.PoSrl !== undefined ?
                                    <Button
                                        onClick={addNewHandler}
                                        variant="outline-success"
                                    >
                                        ايجاد ليست قيمت گذاري
                                    </Button> : <></>}
...
const mapStateToProps = state => {
    return {
        ...
        PricingItem: state.passIds.PricingItem,
    };
};
export default connect(mapStateToProps)(PopupCurdPricing);