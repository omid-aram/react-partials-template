...
const PopupCurdSefareshList = (props) => {
    ...
    const formSubmitHandler = (data) => {
        ...
        baseService.post(url, data).then((result) => {
            ...
            if (result.succeed) {
                debugger;
                // console.log("SEFARESHHHHHHHHHHHHHHH", data)
                // get info for detail cost: SefareshHazine
                baseService.post("/Oaplistt/Show_Order", { LIST_NO: parseInt(data.LIST_NO) }).then((res) => {
                    if (res.succeed) {
                        // console.log("APIDATAAAAA", res)
                        let upperCaseObj = []
                        ...
                        // console.log("upperCaseObj", upperCaseObj);
                        //dispatch(passIdsActions.fetchListSrl(upperCaseObj[0]));
                        // localStorage.setItem('SefareshId', JSON.stringify(upperCaseObj[0]));
                        // history.push("/out/SefareshHazine");
        ...
        finalColumns.push({
            ...
            template: (item) => (
                <>
                    ...
                    {item.VOU === null ? <DeleteButton onClick={() => deleteHandler(item)} /> : <></>}
                </>
            ),
            width: 10
        })
...
export default PopupCurdSefareshList;