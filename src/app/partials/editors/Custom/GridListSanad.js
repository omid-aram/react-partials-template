...
export default function GridListSanad(props) {
    const {
        columns, //required
    ...
    const HandleDeleteSp = (data) => {
        //  debugger;
        let allData = records;
        let forDelete = data;
        for (var i = 0; i < forDelete.length; i++) {

            //  allData.filter(i => i.SRL === forDelete[i])
            allData = arrayRemove(allData, forDelete[i].SRL);
        }

        setRecords(allData);
        deleteHandler(allData)
        dispatch(snackbarActions.success("لیست انتخابی حذف گردید."));
        console.log("datadata", forDelete);
    }
                ...
                toolbar={
                    (
                        <PortletHeaderToolbar>
                            {dataItem.length > 0 ?
                                <Button onClick={() => HandleDeleteSp(CheckedRow)} variant="outline-danger" >
