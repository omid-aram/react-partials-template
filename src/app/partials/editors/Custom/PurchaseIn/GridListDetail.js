 ...
 function GridListDetail(props) {
    ...
    useEffect(() => {
        if (offline) {
            ...
            baseService2.post(url, f).then(({ data }) => {
                if (data.errors) {
                    //alert or something
                } else {
                    console.log("fff", f);
                    console.log("data",data.oapList);
                    //if has error convert this code to ==>> data.items
                    let dataList = [];
                    data.oapList.items.map((item, i) => (
                        dataList.push(keysToLowerCase(item))
                    ...
                    if (isShowAll()) {
                        ...
                        setTotal(data.oapList.totalCount ? data.oapList.totalCount : null);
     ...
     //for changing selected color of row
     useEffect(() => {
        let temp = [...records]
        setRecords(temp);
    }, [selectedRow])
    ...
    // const HandleDeleteListSp = (checkRow) => {
    ...
    //         let obj = {};
    //         obj.SRL = checkRow[i].SRL;
    //         Srllist.push(obj);

    //     }
    //     checkRow=[];
    //     sendData.SRLS = Srllist;
    //     dispatch(passIdsActions.fetchUnchecked(sendData));
    //     baseService.post("/VwOapGrnSnap/DeleteOaplist", sendData).then(({ data }) => {


    //         console.log("DeleteOaplist", data);
                    ...
                    toolbar={
                        (
                            <PortletHeaderToolbar>
                                <Button
                                    onClick={() => HandleDeleteListSp(checkRow)}
...
const mapStateToProps = state => {
    ...
    return {
        Oaplist: state.passIds.Oaplist,
        Unchecked :state.passIds.Unchecked
    };
};
