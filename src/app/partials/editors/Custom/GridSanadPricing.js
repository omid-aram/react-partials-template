...
const GridSanadPricing = (props) => {
    ...
    const HandleCreateSanadSp = (input) => {
        ...
        let b = {};
        b.SRL = input;
        b.PROFORM_SRL = props.PoSrl;
        baseService.post("/Sanad/Sodor_Sanad_Pricing", b).then(({ data }) => {
    ...
    const detailHandler = (id) => {
        history.push("/out/ShowErrorPricing", { newData: id });
                    ...
                    (
                        <PortletHeaderToolbar>
                            ...
                            {hasDataError && dataItem.length > 0 ?
                             <Button onClick={() => detailHandler(GetId)} variant="danger" >
                                مشاهده خطا
                                    </Button> : <></>}
...
const mapStateToProps = state => {
   // console.log("reduxpricing",state)
    return {
...
export default connect(mapStateToProps)(GridSanadPricing);
