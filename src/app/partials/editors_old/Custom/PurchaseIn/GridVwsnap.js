...
import { PortletHeader, PortletHeaderToolbar } from '../../../content/Portlet';
import baseService2 from '../../../../services/base.service2';


export default function GridVwsnap(props) {
    const {
        columns, //required
    ...
    useEffect(() => {
        ...
        console.log("Show data server:" , data);
        if (offline) {
            ...
            baseService2.post(url, f).then(({ data }) => {
                if (data.errors) {
                    ...
                    if (isShowAll()) {
                        ...
                        dispatch(passIdsActions.fetchUnchecked(dataList));
                        setRecords(dataList);
                        setTotal(data.totalCount);
                    }
                ...
                <TableRow key={i} style={{ ...getStripedStyle(i) }}>
                    {selectable ? (<TableCell key={0} >
                        {singleSelect ?
                            ...
                            {console.log("row:" ,row)}
                            {console.log("rowdd:" ,selectedItems.some(x => x === keyColumn))}
                            <Checkbox onChange={(event) => { onSelectChange(row, event.target.checked) }} checked={selectedItems && selectedItems.some(x => x.GRH_SRL === row.GRH_SRL)} />
                       </>
                        }
                    </TableCell>) : null}
    ...
    // const onSelectChange = (item, isCheck) => {
    //     if (singleSelect) {
    //         if (isCheck) {
    //             setSelected(item)
    //         } else {
