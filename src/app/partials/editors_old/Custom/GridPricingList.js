...
const GridPricingList = (props) => {
    ...
    //  console.log("ItemStatusprops",props)
    ...
    const getStripedStyle = (index, rowId) => {
        console.log("rowId TabPanelListSale", rowId, index)
    ...
    const HandleDeleteSp = (item) => {
        ...
        sendData.P_SRL = props.PoSrl;

        baseService.post("/PricingResid/PricingResidDelete", sendData).then(({ data }) => {

           // console.log("PricingResid_Show", data);
                ...
                toolbar={
                    (
                        <PortletHeaderToolbar>
                            {ItemStatus < 2 ?
                                <Button onClick={() => HandleDeleteSp(CheckRow)} variant="outline-danger" >
...
export default connect(mapStateToProps)(GridPricingList);
