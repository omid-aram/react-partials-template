...
const GridResidWithOutParvaneh = (props) => {
    const {
        columns, //required
    ...
    const CreateListPricing = (item) => {
        ...
        for (var i = 0; i < item.length; i++) {
            ...
            let obj = {};
            obj.GRH_SRL = item[i].GRH_SRL;
            Grhlist.push(obj);

        }

        sendData.GRHSRLS = Grhlist;
        sendData.CERTFICATE_NO_CHK = props.CertificateNo;
        sendData.PROFORM_SRL = props.PoSrl;
        sendData.ADJ_CHK = props.AdjChk;
        baseService.post("/PricingResid/Create_Pricing_List_WithOut_Parvaneh", sendData).then(({ data }) => {

            dispatch(snackbarActions.success("درج ليست رسيدها با موفقيت انجام شد."));
                ...
                toolbar={
                    (
                        <PortletHeaderToolbar>
                            {dataItem.length > 0  ? 
                            <Button onClick={() => CreateListPricing(checkrow)} variant="outline-success" >
                                       ایجاد لیست قیمت گذاری
                                    </Button> : <></>}
                        </PortletHeaderToolbar>
                    )}
            />
                    ...
                    <TableFooter>
                        <TableRow>
                            <TablePagination
                                ...
                                rowsPerPageOptions={hideRowsPerPage ? [] : [10, 50, 100, 250]}
...
const mapStateToProps = state => {
    return {
        ...
        CertificateNo: state.passIds.CertificateNo,
        AdjChk: state.passIds.AdjChk,
    };
};
export default connect(mapStateToProps)(GridResidWithOutParvaneh);
