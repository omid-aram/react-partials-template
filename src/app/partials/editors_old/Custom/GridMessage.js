...
export default function GridMessage(props) {
    const {
        columns, //required
        url, messageData, // one required 
        filter,
        GetId,
        keyColumn,
        selectable,
    ...
    const [offlineData, setOfflineData] = useState([messageData]); // offlineData
    ...
    return (
        ...
        {messageData.length > 0 ? <p style={{background:"red" , padding:"10px" , color:"white"}}>{messageData}</p>  : null }
          ...
          <Table stickyHeader={(fixHeight || messageData) ? true : false} className={classes.table} style={{ tableLayout: 'fixed' }} size="small">
              <TableHead>
